        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {products.map((product) => (
              <Link key={product._id} className="group overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white transition duration-200 ">
                {/* Product Image */}
                <div className="flex h-64 items-center justify-center bg-white p-5">
                  {product.images?.length > 0 ? (
                    <img onClick={() => navigate(`/product/${product._id}`)} src={`http://localhost:3000${product.images[0]}`} alt={product.productName} className="h-full w-full object-contain transition duration-300 group-hover:scale-105" />
                  ) : (
